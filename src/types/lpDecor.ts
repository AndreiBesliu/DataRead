...
export const LP_DECOR_EFFECTS = ['none', 'dots', 'constellation', 'shapes', 'grid'] as const;
...
function decorEngine(id: string, cfgJson: string): string {
  return (
    ...
    'function rnd(a,b){return a+Math.random()*(b-a);}var SH=["circle","square","tri"];' +
    ...
    'function build(){P=[];if(eff==="grid")return;var n=eff==="shapes"?Math.round((C.density||40)/100*40):Math.round((C.density||40)/100*120);n=Math.max(6,n);for(i=0;i<n;i++){P.push({x:Math.random()*W,y:Math.random()*H,vx:rnd(-0.4,0.4)*sf,vy:rnd(-0.4,0.4)*sf,r:eff==="shapes"?rnd(base*2,base*6):rnd(base*0.6,base*1.7),s:SH[i%3],rot:Math.random()*6.28,vr:rnd(-0.01,0.01)*sf});}}' +
    ...
    'for(i=0;i<P.length;i++){var p=P[i],dx=p.x+ox,dy=p.y+oy;if(eff==="shapes"){ctx.save();ctx.translate(dx,dy);ctx.rotate(p.rot);ctx.beginPath();if(p.s==="circle"){ctx.arc(0,0,p.r,0,6.283);}else if(p.s==="square"){ctx.rect(-p.r,-p.r,p.r*2,p.r*2);}else{poly(0,0,p.r,3);}ctx.fill();ctx.restore();}else{ctx.beginPath();ctx.arc(dx,dy,p.r,0,6.283);ctx.fill();}}' +
    ...
    'function step(){t+=16;for(i=0;i<P.length;i++){var p=P[i];p.x+=p.vx;p.y+=p.vy;p.rot+=p.vr;if(p.x<-12)p.x=W+12;if(p.x>W+12)p.x=-12;if(p.y<-12)p.y=H+12;if(p.y>H+12)p.y=-12;if(C.interaction==="mouseReact"&&mx>-9000){var ddx=p.x-mx,ddy=p.y-my,dd=ddx*ddx+ddy*ddy;if(dd<120*120&&dd>0.5){var dist=Math.sqrt(dd),f=(1-dist/120)*2.4;p.x+=ddx/dist*f;p.y+=ddy/dist*f;}}}}' +
